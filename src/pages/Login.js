...
export default function Login() {
    ...
    const handleSubmit = (e) => {
        ...
        if (phoneNumberEntered) {
            ...
            const userExists = registeredUsers.some(user => user.phone === phoneNumber);
            if (userExists) {
    ...
    return (
        <div className="relative w-full h-screen bg-yellow-400/85">
            <img className="absolute w-full h-full object-cover mix-blend-overlay opacity-60" src={loginImg} alt="/" />
            <div className="flex flex-col justify-center items-center h-full">
                <img src={companyLogo} alt="Company Logo" className="h-8 md:h-12 mt-8 mb-2" />
                <form onSubmit={handleSubmit} className="md:max-w-[750px] max-w-[364px] w-full mx-auto bg-yellow-400 p-8 border border-gray-300 rounded-xl mt-10">
                    <div className="flex flex-col mb-6 relative">
                        <label htmlFor="phone" className="text-gray-950 font-semibold text-base md:text-lg mb-3">Masukkan nomor telepon anda</label>
                        <input
                            id="phone"
                            placeholder="+62"
                            value={phoneNumber}
                    ...
                    {warningShown && !phoneNumberEntered && (
                        <p className="text-red-500 text-sm mb-4">Harap masukkan nomor telepon terlebih dahulu</p>
                    )}
                    {warningShown && phoneNumberEntered && (
                        <p className="text-red-500 text-sm mb-4">Nomor telepon tidak terdaftar</p>
                    )}
                    <button className={`w-full px-4 py-4 md:mt-10 bg-black hover:bg-yellow-500 rounded-2xl relative text-white font-bold`}>Masuk</button>
                </form>
            </div>
        </div>
    )
